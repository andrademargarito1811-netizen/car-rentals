                        ...
                        <div className="relative flex items-center justify-between flex-wrap gap-4">
                            <div className="space-y-1">
                                ...
                                <p className="text-white/60 max-w-xl">
                                    Manage all user and admin accounts.
                                </p>
                            </div>
                ...
                <div className="pb-8 sm:pb-12 pt-4 sm:pt-6">
                    <div className="px-6 lg:px-10 space-y-8">
                        ...
                        <div className="grid gap-4 sm:grid-cols-4">
                            {[
                                { label: 'Total Accounts', value: stats.total, gradient: 'from-brand-500/20 to-brand-700/10', iconGradient: 'from-brand-500 to-brand-600' },
                                { label: 'Active', value: stats.active, gradient: 'from-emerald-500/20 to-emerald-600/10', iconGradient: 'from-emerald-500 to-emerald-600' },
                                { label: 'Admins', value: stats.admins, gradient: 'from-blue-500/20 to-blue-600/10', iconGradient: 'from-blue-500 to-blue-600' },
                                { label: 'Suspended', value: stats.suspended, gradient: 'from-red-500/20 to-red-600/10', iconGradient: 'from-red-500 to-red-600' },
                            ].map((stat, i) => (
                        ...
                        <div className="space-y-5">
                            <div className="flex items-center gap-4">
                                <span className="text-[11px] font-semibold uppercase tracking-[0.15em] text-surface-400 dark:text-surface-500 shrink-0">Accounts</span>
                                {hasActiveFilters && <span className="text-[11px] text-surface-400 dark:text-surface-500">({users.data.length} result{users.data.length !== 1 ? 's' : ''})</span>}
                                        ...
                                        <thead>
                                            <tr>
                                                <th onClick={() => handleSort('name')}
                                                    ...
                                                    Name<span className="text-[9px]">{sortArrow('name')}</span>
                                                </th>
                                                <th onClick={() => handleSort('email')}
                                                    className="px-5 py-4 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-surface-400 dark:text-surface-500 bg-surface-50/50 dark:bg-surface-800/30 border-b border-surface-100 dark:border-surface-700/50 cursor-pointer select-none hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
                                                    Email<span className="text-[9px]">{sortArrow('email')}</span>
                                                </th>
                                                <th onClick={() => handleSort('role')}
                                                    className="px-5 py-4 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-surface-400 dark:text-surface-500 bg-surface-50/50 dark:bg-surface-800/30 border-b border-surface-100 dark:border-surface-700/50 cursor-pointer select-none hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
                                                    Role<span className="text-[9px]">{sortArrow('role')}</span>
                                                </th>
                                                <th onClick={() => handleSort('status')}
                                                    className="px-5 py-4 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-surface-400 dark:text-surface-500 bg-surface-50/50 dark:bg-surface-800/30 border-b border-surface-100 dark:border-surface-700/50 cursor-pointer select-none hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
                                                    Status<span className="text-[9px]">{sortArrow('status')}</span>
                                                </th>
                                                <th onClick={() => handleSort('created_at')}
                                                    className="px-5 py-4 text-left text-[10px] font-bold uppercase tracking-[0.12em] text-surface-400 dark:text-surface-500 bg-surface-50/50 dark:bg-surface-800/30 border-b border-surface-100 dark:border-surface-700/50 cursor-pointer select-none hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
                                                    Created<span className="text-[9px]">{sortArrow('created_at')}</span>
                                                </th>
                                            ...
                                            {users.data.length === 0 ? (
                                                <tr>
                                                    <td colSpan={6} className="px-5 py-20 text-center">
                                                        ...
                                                        <p className="text-surface-500 dark:text-surface-400 font-medium">No accounts found</p>
                                                    ...
                                                    <tr key={user.id}
                                                        ...
                                                        <td className="px-5 py-4 relative">
                                                            ...
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <span className="font-semibold text-surface-900 dark:text-white text-sm">{user.name}</span>
                                                            </div>
                                                        </td>
                                                        ...
                                                        <td className="px-5 py-4">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <span className="text-sm text-surface-600 dark:text-surface-400 truncate max-w-[200px]">{user.email}</span>
                                                            </div>
                                                            {user.phone && (
                                                                <div className="flex items-center gap-2 mt-1">
                                                                    ...
                                                                    <span className="text-[12px] text-surface-400 dark:text-surface-500">{user.phone}</span>
                                                                </div>
                                                            )}
                                                        </td>
                                                        ...
                                                        <td className="px-5 py-4">
                                                            <Badge variant={user.role === 'admin' ? 'default' : 'secondary'} className="capitalize">
                                                                {user.role === 'admin' && (
                                                                    <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                                        ...
                                                        {/* Status */}
                                                        <td className="px-5 py-4">
                                                            <span className={user.status === 'active' ? 'badge-active' : 'badge-cancelled'}>
                                                                {user.status}
                                                            </span>
                                                        </td>
