...
interface ExtraChargeIndexProps {
    extraCharges: {
        data: ExtraCharge[];
...
export default function ExtraChargeIndex({ extraCharges }: ExtraChargeIndexProps) {
    const route = useRoute();
    const [showPanel, setShowPanel] = useState(false);
    const [editingCharge, setEditingCharge] = useState<ExtraCharge | null>(null);
    const [showDeleteModal, setShowDeleteModal] = useState(false);
    const [deletingCharge, setDeletingCharge] = useState<ExtraCharge | null>(null);
    ...
    function openEdit(charge: ExtraCharge) {
        ...
        setData({
            ...
            apply_always: charge.apply_always,
    ...
    function submitForm(e: React.FormEvent) {
        e.preventDefault();
        if (editingCharge) {
            put(route('admin.extra-charges.update', editingCharge.id), {
    ...
    const activeCount = extraCharges.data.filter((c) => c.is_active).length;
    const chargeCount = extraCharges.data.filter((c) => c.type === 'Extra Charge').length;
    const discountCount = extraCharges.data.filter((c) => c.type === 'Discount').length;

    const headerGradient = 'from-brand-600 to-brand-800 dark:from-brand-700 dark:to-brand-900';

    return (
        <>
            <Head title="Extra Charges" />
            <AuthenticatedLayout
                breadcrumbs={[{ label: 'Extra Charges' }]}
                header={
                        ...
                        <div className="relative flex items-center justify-between flex-wrap gap-4">
                            <div className="space-y-1">
                                ...
                                <p className="text-white/60 max-w-xl">
                                    Configure optional add-on charges (e.g. CDW) applied when a vehicle is returned.
                                </p>
                            </div>
                ...
                <div className="pb-8 sm:pb-12 pt-4 sm:pt-6">
                    <div className="px-6 lg:px-10 space-y-8">
                        ...
                        <div className="grid gap-4 sm:grid-cols-4">
                            {[
                                { label: 'Total', value: extraCharges.data.length, gradient: 'from-brand-500/20 to-brand-700/10', iconGradient: 'from-brand-500 to-brand-600', icon: 'M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z' },
                                ...
                                { label: 'Charges', value: chargeCount, gradient: 'from-amber-500/20 to-amber-600/10', iconGradient: 'from-amber-500 to-amber-600', icon: 'M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z' },
                        ...
                        <div className="space-y-5">
                            <div className="flex items-center gap-4">
                                <span className="text-[11px] font-semibold uppercase tracking-[0.15em] text-surface-400 dark:text-surface-500 shrink-0">Charge Rules</span>
                                <span className="flex-1 h-px bg-gradient-to-r from-surface-200 dark:from-surface-700 to-transparent" />
                            </div>

                            {extraCharges.data.length === 0 ? (
                                <div className="rounded-xl bg-white dark:bg-brand-800 border border-surface-100 dark:border-surface-700/60 p-12 sm:p-16 animate-fade-in-up">
                                    <div className="text-center max-w-sm mx-auto">
                                        ...
                                        <h3 className="text-lg font-bold text-surface-900 dark:text-white mb-1">No extra charges found</h3>
                                        <p className="text-sm text-surface-500 dark:text-surface-400 mb-6">Create your first extra charge (e.g. CDW) to get started.</p>
                                        <button onClick={openCreate}
                                        ...
                                        <table className="w-full text-sm">
                                            <thead>
                                                <tr className="border-b border-surface-100 dark:border-surface-700/60 bg-surface-50/70 dark:bg-brand-900/30">
                                                    <th className="text-left px-5 py-3.5 text-[11px] font-semibold uppercase tracking-wider text-surface-500 dark:text-surface-400">Name</th>
                                                    <th className="text-left px-5 py-3.5 text-[11px] font-semibold uppercase tracking-wider text-surface-500 dark:text-surface-400">Type</th>
                                                    ...
                                                    <th className="text-left px-5 py-3.5 text-[11px] font-semibold uppercase tracking-wider text-surface-500 dark:text-surface-400">Taxable</th>
                                            ...
                                            <tbody className="divide-y divide-surface-100 dark:divide-surface-700/40">
                                                {extraCharges.data.map((charge, i) => (
                                                    <tr key={charge.id} className={`animate-fade-in-up stagger-${(i % 6) + 1} group hover:bg-surface-50/50 dark:hover:bg-brand-900/20 transition-colors duration-150`}>
                                                        <td className="px-5 py-4">
                                                            <div className="flex items-center gap-3">
                                                                <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${charge.is_active ? 'bg-emerald-500' : 'bg-surface-300 dark:bg-surface-600'}`} />
                                                                <div>
                                                                    <p className="font-semibold text-surface-900 dark:text-white">{charge.name}</p>
                                                                    <div className="flex items-center gap-1.5 mt-1">
                                                                        <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-bold rounded ${charge.operator === '+' ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/25' : 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/25'}`}>
                                                        ...
                                                        <td className="px-5 py-4">
                                                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded ${typeBadge(charge.type)}`}>
                                                                {charge.type}
                                                            </span>
                                                        </td>
                                                        <td className="px-5 py-4">
                                                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded ${calculationBadge(charge.calculation)}`}>
                                                                {charge.calculation}
                                                            </span>
                                                        </td>
                                                        <td className="px-5 py-4">
                                                            <span className="font-mono font-semibold text-surface-900 dark:text-white">
                                                                {charge.value_in === 'Percentage' ? `${parseFloat(charge.rate).toFixed(2)}%` : `$${parseFloat(charge.rate).toFixed(2)}`}
                                                            </span>
                                                            <span className="text-[10px] text-surface-400 dark:text-surface-500 ml-1">
                                                                {charge.value_in === 'Percentage' ? 'Percentage' : 'Amount'}
                                                            </span>
                                                        </td>
                                                        <td className="px-5 py-4">
                                                        ...
                                                        <td className="px-5 py-4 text-right">
                                                            <div className="flex items-center justify-end gap-1 opacity-100 transition-opacity duration-150">
                                                                ...
                                                                <button onClick={() => { setDeletingCharge(charge); setShowDeleteModal(true); }}
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-sm bg-white dark:bg-brand-800 rounded-2xl shadow-2xl shadow-black/20 border border-surface-100 dark:border-surface-700/60 animate-fade-in-up overflow-hidden">
                        ...
                        <div className="relative p-6 sm:p-7 text-center">
                            ...
                            <h3 className="text-lg font-bold text-surface-900 dark:text-white mb-1">Delete Extra Charge</h3>
                            <p className="text-sm text-surface-500 dark:text-surface-400 mb-2">
                                Are you sure you want to delete
                            </p>
                            <p className="text-sm font-semibold text-surface-700 dark:text-surface-200 mb-6">
                                &ldquo;{deletingCharge.name}&rdquo;?
                            </p>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => {
                                    router.delete(route('admin.extra-charges.destroy', deletingCharge.id), {
            ...
            <SlidePanel
                show={showPanel}
                onClose={closePanel}
                title={editingCharge ? 'Edit Extra Charge' : 'Create Extra Charge'}
            >
                <ExtraChargeForm
                    data={data}
                    setData={setData}
