...
function CarouselPreview({ images, badgeText, badgeIcon, headline, headlineHighlight, tagline, description }: {
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => setSlide(s => (s + 1) % images.length), 3500);
    ...
    return (
        <div className="relative w-full h-56 sm:h-72 rounded-2xl overflow-hidden bg-brand-900 shadow-elevated">
            ...
            <div className="relative z-10 p-5 sm:p-7 h-full flex flex-col justify-center max-w-sm">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-accent-400/15 text-accent-300 text-[10px] font-semibold rounded-full border border-accent-400/25 mb-3 w-fit backdrop-blur-sm">
                ...
                {tagline && <p className="text-accent-400 font-medium text-xs mt-1.5 tracking-wide">{tagline}</p>}
                {description && <p className="text-surface-400 text-[11px] mt-1.5 leading-relaxed">{description}</p>}
            </div>
        ...
        <div className="space-y-4">
            <div className="flex items-center gap-2.5">
                ...
                <div>
                    ...
                    <p className="text-[10px] text-surface-500 dark:text-surface-400">Hero banners as they appear on each page</p>
                </div>
            </div>
                ...
                {images.length > 0 ? (
                    ...
                    <div className="relative w-full h-48 rounded-none overflow-hidden bg-brand-900">
                        ...
                        <div className="relative z-10 p-4 h-full flex flex-col justify-center max-w-xs">
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-accent-400/15 text-accent-300 text-[9px] font-semibold rounded-full border border-accent-400/25 mb-2 w-fit backdrop-blur-sm">
                                {badgeIcon && BADGE_ICON_PATHS[badgeIcon] ? (
                            ...
                            {tagline && <p className="text-accent-400 font-medium text-[10px] mt-1">{tagline}</p>}
                            {description && <p className="text-surface-400 text-[10px] mt-1 leading-relaxed line-clamp-2">{description}</p>}
                        </div>
                    </div>
                )}
            </div>

            {/* Fleet page hero preview */}
            <div className="rounded-xl border border-surface-200 dark:border-surface-700/60 overflow-hidden">
                <div className="px-3 py-2 bg-surface-50 dark:bg-surface-800/50 border-b border-surface-200 dark:border-surface-700/60">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-surface-500">Fleet Page</span>
                </div>
                <div className="relative w-full h-36 rounded-none overflow-hidden bg-brand-900">
                    {fleetImage && (
                        <img src={fleetImage} alt="" className="absolute inset-0 w-full h-full object-cover" />
                    )}
                    <div className={cn(
                        'absolute inset-0',
                        fleetImage
                            ? 'bg-gradient-to-r from-brand-900/95 via-brand-900/80 to-brand-900/50'
                            : 'bg-gradient-to-br from-brand-800 to-brand-900'
                    )} />
                    ...
                    <div className="relative z-10 p-4 h-full flex flex-col justify-center">
                        ...
                        <p className="text-surface-400 text-[10px] mt-1 leading-relaxed max-w-xs">
