...
export default function AdminCarsCreate({ locations = [], classes = [], availabilities = [], recentVehicles = [] }: { locations?: { location_id: number; location: string }[]; classes?: { class_no: string; class_desc: string }[]; availabilities?: { available_id: number; available_desc: string }[]; recentVehicles?: RecentVehicle[] }) {
    ...
    const form = useForm({
        ...
        fuel_type: 'gasoline', seats: '', transmission: 'automatic',
        description: '', image: null as File | null,
        location_id: '', stock_number: '', vin: '', color: '',
    ...
    function validate(): boolean {
        ...
        for (const [field, label] of Object.entries(REQUIRED_FIELDS)) {
            ...
            if (field === 'image') {
                ...
                val = (d as Record<string, unknown>)[field];
            }
    ...
    function submit() {
        if (!validate()) return;
        form.post(route('admin.cars.store'), { forceFormData: true });
    }
    ...
    const carName = [d.brand, d.model, d.year].filter(Boolean).join(' ') || 'New Vehicle';
    const rate = d.daily_rate ? `$${parseFloat(d.daily_rate).toFixed(2)}` : '—';
    const specCount = [d.seats, d.transmission, d.fuel_type, d.vehicle_doors].filter(Boolean).length;
    ...
    return (
        <>
            <Head title="Add Car" />
            <AuthenticatedLayout
                breadcrumbs={[{ label: 'Cars', href: 'admin.cars.index' }, { label: 'Add Car' }]}
                header={
                        ...
                        <div className="flex items-center gap-5 relative z-10">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 flex-wrap">
                                    <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">Add New Car</h2>
                                ...
                                <p className="text-sm text-brand-200/80 mt-0.5">Fill in the details below to add a new vehicle to your fleet</p>
                            </div>
                            <div className="hidden sm:flex items-center gap-3">
                                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 backdrop-blur-sm ring-1 ring-white/10">
                                    ...
                                    <span className="text-[11px] font-medium text-brand-200/80">Draft</span>
                                </div>
                        ...
                        <div className="card overflow-hidden">
                            <form onSubmit={submit} encType="multipart/form-data">
                                <div className="flex flex-col xl:flex-row">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-4 mb-6 p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-surface-800/40 dark:to-surface-800/10 border border-amber-200/70 dark:border-surface-700/40 animate-fade-in-up">
                                            <div className="flex items-center gap-4 shrink-0">
                                                ...
                                                <div className="sm:hidden">
                                                    <span className="text-xs font-semibold uppercase tracking-wider text-amber-700">Photo <span className="text-red-500">*</span></span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between mt-8 pt-5 border-t border-amber-200/60 dark:border-surface-700/50 animate-fade-in-up">
                                            ...
                                            <button type="submit" className="btn-primary group" disabled={form.processing}>
                                                {form.processing ? (
                                                    ...
                                                    <span className="flex items-center gap-2">
                                                        <svg className="w-4 h-4 group-hover:scale-110 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                                                        Create Car
                                                    </span>
                                                )}
                                            </button>
                                    ...
                                    {/* ── Sticky Sidebar: Business Card ── */}
                                    <div className="xl:w-80 shrink-0 border-t xl:border-t-0 xl:border-l border-amber-200/60 dark:border-surface-700/50 bg-gradient-to-b from-amber-50 to-amber-100/30 dark:from-surface-900/30 dark:to-surface-800/10">
                                        <div className="xl:sticky xl:top-0 p-5 space-y-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-semibold uppercase tracking-widest text-amber-700">Preview</span>
                                            </div>

                                            {/* The card */}
                                            <div className="rounded-2xl bg-amber-50 dark:bg-surface-800/80 border border-amber-300/50 dark:border-surface-700/50 shadow-sm overflow-hidden card-shine group">
                                                ...
                                                <div className="p-4 space-y-3">
                                                    <div className="flex items-start justify-between">
                                                        <div className="min-w-0">
                                                            ...
                                                            {d.license_plate && (
                                                                <p className="text-[11px] text-amber-700 dark:text-surface-400 font-mono mt-0.5">{d.license_plate}</p>
                                                            )}
                                                        </div>
                                                    ...
                                                    <div className="flex items-baseline gap-1">
                                                        ...
                                                        {d.daily_rate && <span className="text-[11px] text-amber-700 dark:text-surface-400">/day</span>}
                                                    </div>

                                                    {(d.seats || d.transmission || d.fuel_type) && (
                                                        <div className="flex flex-wrap gap-1.5 pt-2 border-t border-amber-200/60 dark:border-surface-700/50">
                                                            {d.seats && <span className="px-2.5 py-1 rounded-md bg-amber-100/60 dark:bg-surface-700/50 text-[10px] font-medium text-amber-800 dark:text-surface-400 transition-all duration-200 hover:bg-brand-100 dark:hover:bg-brand-900/30 hover:text-brand-700 dark:hover:text-brand-400">{d.seats} seats</span>}
                                                            {d.transmission && <span className="px-2.5 py-1 rounded-md bg-amber-100/60 dark:bg-surface-700/50 text-[10px] font-medium text-amber-800 dark:text-surface-400 capitalize transition-all duration-200 hover:bg-brand-100 dark:hover:bg-brand-900/30 hover:text-brand-700 dark:hover:text-brand-400">{d.transmission}</span>}
                                                            {d.fuel_type && <span className="px-2.5 py-1 rounded-md bg-amber-100/60 dark:bg-surface-700/50 text-[10px] font-medium text-amber-800 dark:text-surface-400 capitalize transition-all duration-200 hover:bg-brand-100 dark:hover:bg-brand-900/30 hover:text-brand-700 dark:hover:text-brand-400">{d.fuel_type}</span>}
                                                        </div>
                                                    )}

                                                    <div className="pt-2 border-t border-amber-200/60 dark:border-surface-700/50 flex items-center justify-between">
                                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold bg-amber-100/60 dark:bg-surface-700/50 text-amber-800 dark:text-surface-400">
                                                            {availabilities.find(a => a.available_id === Number(d.availability_id))?.available_desc || 'Not set'}
                                                        </span>
                                                        <span className="text-[10px] text-amber-700 dark:text-surface-400">{totalPercent}% complete</span>
                                                    </div>
                                                </div>
                                            </div>

                                            {/* Quick Stats Grid */}
                                            <div className="rounded-xl bg-amber-50/80 dark:bg-surface-800/40 border border-amber-200/60 dark:border-surface-700/40 p-3">
                                                <div className="text-[10px] font-semibold uppercase tracking-wider text-amber-700 mb-2.5">Quick Stats</div>
                                                <div className="grid grid-cols-2 gap-2">
                                                    {[
                                                        ...
                                                        <div key={stat.label} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-amber-50 dark:bg-surface-800/60">
                                                            <svg className="w-3 h-3 shrink-0 text-amber-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                                                <path strokeLinecap="round" strokeLinejoin="round" d={stat.icon} />
                                                            </svg>
                                                            <div className="min-w-0">
                                                                <div className="text-[10px] text-amber-700 dark:text-surface-400 leading-tight">{stat.label}</div>
                                                                <div className="text-[11px] font-semibold text-amber-900 dark:text-surface-200 leading-tight truncate">{stat.value}</div>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                            ...
                                            <div className="rounded-xl bg-amber-50/80 dark:bg-surface-800/40 border border-amber-200/60 dark:border-surface-700/40 p-3">
                                                <div className="text-[10px] font-semibold uppercase tracking-wider text-amber-700 mb-2.5">Sections</div>
                                                <div className="space-y-1.5">
                                                    {SECTIONS.map(s => {
                                                        ...
                                                        return (
                                                            <div key={s.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-amber-50 dark:bg-surface-800/60">
                                                                <span className={`shrink-0 w-4 h-4 rounded flex items-center justify-center transition-all duration-300 ${
                                                                    done ? 'bg-emerald-500' : partial ? 'bg-brand-400' : 'bg-amber-200 dark:bg-surface-700'
                                                                }`}>
                                                                    {done ? (
                                                                ...
                                                                <span className="flex-1 text-[11px] font-medium text-amber-700 dark:text-surface-300">{s.title}</span>
                                                                <span className={`text-[10px] font-medium ${
                                                                    done ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-700'
                                                                }`}>{sp.percent}%</span>
                                                            </div>
                                                        );
                                                    })}
                                            ...
                                            <div className="rounded-xl bg-amber-50/80 dark:bg-surface-800/40 border border-amber-200/60 dark:border-surface-700/40 p-3">
                                                <div className="text-[10px] font-semibold uppercase tracking-wider text-amber-700 mb-2.5">Field Summary</div>
                                                {(() => {
                                                    const allFields = Object.values(SECTION_FIELDS).flat();
                                                    ...
                                                    return (
                                                        <div className="space-y-1.5">
                                                            <div className="flex items-center justify-between px-2 py-1 rounded bg-amber-50 dark:bg-surface-800/60">
                                                                <span className="text-[11px] text-amber-800 dark:text-surface-400">Total fields</span>
                                                                <span className="text-[11px] font-semibold text-amber-900 dark:text-surface-200">{total}</span>
                                                            </div>
                                                            <div className="flex items-center justify-between px-2 py-1 rounded bg-amber-50 dark:bg-surface-800/60">
                                                                <div className="flex items-center gap-1.5">
                                                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                                                                    <span className="text-[11px] text-amber-800 dark:text-surface-400">Filled</span>
                                                                </div>
                                                                <span className="text-[11px] font-semibold text-emerald-600 dark:text-emerald-400">{filled}</span>
                                                            </div>
                                                            <div className="flex items-center justify-between px-2 py-1 rounded bg-amber-50 dark:bg-surface-800/60">
                                                                <div className="flex items-center gap-1.5">
                                                                    <span className="w-1.5 h-1.5 rounded-full bg-amber-300 dark:bg-surface-600" />
                                                                    <span className="text-[11px] text-amber-800 dark:text-surface-400">Empty</span>
                                                                </div>
                                                                <span className="text-[11px] font-semibold text-amber-700">{empty}</span>
                                                            </div>
                                                            {errors > 0 && (
                                                                <div className="flex items-center justify-between px-2 py-1 rounded bg-red-50 dark:bg-red-900/20">
                                                                    <div className="flex items-center gap-1.5">
                                                                        <span className="w-1.5 h-1.5 rounded-full bg-red-500" />
                                                                        <span className="text-[11px] text-red-600 dark:text-red-400">Errors</span>
                                                                    </div>
                                                                    <span className="text-[11px] font-semibold text-red-600 dark:text-red-400">{errors}</span>
                                                                </div>
                                                            )}
                                                        </div>
                                                    );
                                            ...
                                            <div className="rounded-xl bg-amber-50/80 dark:bg-surface-800/40 border border-amber-200/60 dark:border-surface-700/40 p-3">
                                                <div className="text-[10px] font-semibold uppercase tracking-wider text-amber-700 mb-2.5">Recently Added</div>
                                                {recentVehicles.length > 0 ? (
                                                    <div className="space-y-1.5">
                                                        {recentVehicles.slice(0, 3).map(v => (
                                                            <Link
                                                                key={v.id}
                                                                href={route('admin.cars.edit', v.id)}
                                                                className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg bg-amber-50 dark:bg-surface-800/60 hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors group"
                                                            >
                                                                <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-brand-100 to-brand-200 dark:from-brand-800 dark:to-brand-900 flex items-center justify-center shrink-0">
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="text-[11px] font-medium text-amber-900 dark:text-surface-200 truncate group-hover:text-brand-700 dark:group-hover:text-brand-400 transition-colors">
                                                                        {v.brand} {v.model} ({v.year})
                                                                    </div>
                                                                    {v.license_plate && (
                                                                        <div className="text-[10px] text-amber-700 dark:text-surface-400 font-mono">{v.license_plate}</div>
                                                                    )}
                                                                </div>
                                                                <svg className="w-3 h-3 text-amber-700 dark:text-surface-600 group-hover:text-brand-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                                                                </svg>
                                                            </Link>
                                                        ))}
                                                    ...
                                                    <div className="flex flex-col items-center gap-1.5 py-3 text-amber-700">
                                                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                                                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                                                        </svg>
                                                        <span className="text-[10px]">No vehicles yet</span>
                                                        <span className="text-[9px] text-amber-600 dark:text-surface-500">Save your first car to see it here</span>
                                                    </div>
                                                )}
                                            </div>

                                            <p className="text-[10px] text-amber-700 dark:text-surface-400 text-center leading-relaxed animate-soft-pulse">
                                                Preview updates automatically<br />as you fill in the form
                                            </p>
                                        </div>
                                    </div>
                                </div>
