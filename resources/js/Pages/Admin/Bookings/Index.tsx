...
interface BookingRecord {
    ...
    car: { id: number; brand: string; model: string; license_plate: string } | null;
    created_at: string;
}
...
function getCustomerName(b: BookingRecord): string {
    return b.user?.name ?? (b.guest ? `${b.guest.first_name} ${b.guest.last_name}` : 'Guest');
}

function getInitials(b: BookingRecord): string {
    const name = getCustomerName(b);
    return name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
}

export default function AdminBookingsIndex({ bookings, filters }: AdminBookingsIndexProps) {
    useBookingBroadcast();
    ...
    function updateFilter(status: string) {
        router.get(route('admin.bookings.index'), { status, search }, { preserveState: true, preserveScroll: true });
    }
                        ...
                        <div className="relative flex items-center justify-between flex-wrap gap-4">
                            <div className="space-y-1">
                                ...
                                <p className="text-white/60 max-w-xl">
                                    View, filter, and manage all customer reservations and bookings.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="pb-8 sm:pb-12 pt-4 sm:pt-6">
                    <div className="px-6 lg:px-10 space-y-8">
                        ...
                        <div className="grid gap-4 sm:grid-cols-5">
                            {[
                                { label: 'Total', value: statusCounts.total, gradient: 'from-brand-500/20 to-brand-700/10', iconGradient: 'from-brand-500 to-brand-600', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
                                            ...
                                            {bookings.data.length === 0 ? (
                                                <tr>
                                                    <td colSpan={7} className="px-5 py-20 text-center">
                                                        ...
                                                        <p className="text-surface-500 dark:text-surface-400 font-medium">No bookings found</p>
                                                        <p className="text-sm text-surface-400 dark:text-surface-500 mt-1">No bookings match your current filters.</p>
                                                    </td>
                                                </tr>
                                            ) : (
